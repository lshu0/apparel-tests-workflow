...
paths=[]
wd = "C:/Users/lshu0/Documents/Teradata driver"  # change to directory of all excel files
pattern   = "*.xls"
for x in os.walk(wd):
...
week = '03/26/2016'  # change to date of Saturday in that week
wk_no = 8  # change to week number
total_site_num = 685 
...
for path in paths:
    ...
    for sheet in sheets:
        ...
        df['expected']=df['Test actual (APT-only)']-df['Estimated impact']
        df['actual']=df['Test actual (APT-only)']
