...
from	(select	pa11.WK_NBR  WK_NBR,
	...
	from	(select	a11.WK_NBR  WK_NBR,
		...
		where	(a11.WK_NBR in (201608)
		 and a11.LOCN_NBR in (sel locn from shc_work_tbls.Sears_all_stores where wk_no = 8 and test_nm = 'Basics 1' )
	...
	where	(a13.MRCH_NBR in (20000)
	 ...
	 and a13.DIVITEM in (sel trim(div_no)||'-'||trim(itm_no) from shc_work_tbls.sears_all_items where wk_no = 8 and test_nm = 'Basics 1'))
	...
	where	(a12.MRCH_NBR in (20000)
	 ...
	 and a11.LOCN_NBR in (sel locn from  shc_work_tbls.Sears_all_stores where wk_no = 8 and test_nm = 'Basics 1')
	 and a12.DIVITEM in (sel trim(div_no)||'-'||trim(itm_no) from shc_work_tbls.sears_all_items where wk_no = 8 and test_nm = 'Basics 1')
	 and a11.TRS_TYP_CD in ('A', 'R', 'S')
	...
	where	(s21.MRCH_NBR in (20000)
	 and s21.DIVITEM in (sel trim(div_no)||'-'||trim(itm_no) from shc_work_tbls.sears_all_items where wk_no = 8 and test_nm = 'Basics 1'))
