...
from	(select	pa11.WK_NBR  WK_NBR,
	...
	from	(select	a11.WK_NBR  WK_NBR,
		...
		where	(a11.WK_NBR in (201607)
		 and a11.LOCN_NBR in ( sel locn from shc_work_tbls.Sears_all_stores where wk_no = 7 and test_nm = 'Basics 2' )
	...
	where	(a13.MRCH_NBR in (20000)
	 ...
	 and pa11.LOCN_NBR in (sel locn from  shc_work_tbls.Sears_all_stores where wk_no = 7 and test_nm = 'Basics 2' )
...
where	(coalesce(pa11.WK_NBR, pa12.WK_NBR) in (201607)
 and coalesce(pa11.LOCN_NBR, pa12.LOCN_NBR) in (sel locn from shc_work_tbls.Sears_all_stores where wk_no = 7 and test_nm = 'Basics 2' ))
